...
const Verify = () => {
  ...
  // ✅ FIX: Prevents React 18 Strict Mode from calling the API twice and throwing a false "Invalid Token" error
  ...
  useEffect(() => {
    ...
    if (token && !hasFetched.current) {
      hasFetched.current = true; // Mark as fetched so it doesn't run twice
      verifyAccount();
    }
  }, [token]);
      ...
      <style>{`
        * {
        ...
        .verify-page {
          ...
          background: linear-gradient(135deg, #050505 0%, #111111 50%, rgba(255, 215, 0, 0.05) 100%);
        ...
        /* ✅ Box styling removed entirely. It now floats cleanly in the center. */
        .content-wrapper {
          width: 100%;
          max-width: 420px;
          text-align: center;
          ...
          <AnimatePresence mode="wait">
            ...
            {status === "pending" && (
              <motion.div key="pending" variants={contentVariants} initial="hidden" animate="visible" exit="exit">
                ...
                <h2 className="title pending-title">Check Your Inbox</h2>
                <p className="message">
            ...
            {status === "invalid" && (
              <motion.div key="invalid" variants={contentVariants} initial="hidden" animate="visible" exit="exit">
                ...
                <h2 className="title error-title">Invalid Request</h2>
            ...
            {status === "loading" && (
              <motion.div key="loading" variants={contentVariants} initial="hidden" animate="visible" exit="exit">
                ...
                <h2 className="title loading-title">Verifying Account</h2>
            ...
            {status === "success" && (
              <motion.div key="success" variants={contentVariants} initial="hidden" animate="visible" exit="exit">
                ...
                <h2 className="title success-title">Verification Complete</h2>
            ...
            {status === "error" && (
              <motion.div key="error" variants={contentVariants} initial="hidden" animate="visible" exit="exit">
                ...
                <h2 className="title error-title">Verification Failed</h2>
                <p className="message">{message}</p>
